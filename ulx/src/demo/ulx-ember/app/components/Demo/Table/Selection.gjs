...
export default class DemoTableSelection extends Component {
  ...
  <template>
    <div class="ulx-grid cols-1 gp3">
      <div>
        <h4 class="h5 mgb2">Single Row Selection</h4>
        <p class="text-sm fg-text-secondary mgb2">Click a row to select it.</p>
        <UlxTable
          @value={{this.products}}
        ...
        {{#if this.singleSelection}}
          <p class="mgt2 text-sm">Selected: <strong>{{this.singleSelection.name}}</strong></p>
        {{/if}}
      </div>

      <div class="mgt3">
        <h4 class="h5 mgb2">Checkbox Selection</h4>
        <p class="text-sm fg-text-secondary mgb2">Use checkboxes to select multiple rows.</p>
        <UlxTable
          @value={{this.products}}
        ...
        />
        <p class="mgt2 text-sm">Selected: <strong>{{this.multiSelection.length}}</strong> row(s)</p>
      </div>
    </div>
  </template>
}
