...
const defaultExportPattern = /(export default function Utilities\(\) \{[\s\S]*?)(\s+<\/div>\s+\)\s+\})/;
...
const utilitiesRoutePattern = /(this\.route\('utilities', \{ path: '\/utilities' \}, function \(\) \{[\s\S]*?)(\}\);)/;
