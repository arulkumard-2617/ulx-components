...
if (!componentFlag) {
	...
	console.error(
		'Usage: npm run destroy-variation --component=ComponentName --variation=VariationName'
	);
	process.exit(1);
}
...
if (!fs.existsSync(demoComponentPath)) {
	console.error(
		`Error: Variation '${variationPascal}' does not exist for component '${componentName}'`
	);
	process.exit(1);
}
...
// Remove demo export
const demoExportPattern = new RegExp(
	`export\\s*\\{\\s*default\\s+as\\s+${variationPascal}Demo\\s*\\}\\s*from\\s+['"][^'"]+['"];?\\s*\\n?`,
	'g'
);
...
const snippetImportPattern = new RegExp(
	`export\\s*\\{\\s*default\\s+as\\s+${variationPascal}Source\\s*\\}\\s*from\\s+['"][^'"]+['"];?\\s*\\n?`,
	'g'
);
importsContent = importsContent.replace(snippetImportPattern, '');

// Pattern 2: Hardcoded string: export const VariationSource = `...`;
const hardcodedSourcePattern = new RegExp(
	`export\\s+const\\s+${variationPascal}Source\\s*=\\s*\`[^\`]+\`;?\\s*\\n?`,
	'g'
);
...
// Clean up: fix comment formatting in imports (// Sources should be on new line)
featuresContent = featuresContent.replace(/,(\s*)\/\/ Sources/g, ',\n  // Sources');
...
if (idMatch) {
	...
	if (startIndex !== -1 && endIndex !== -1) {
		// Look for preceding comma (might be on previous line)
		...
		for (let i = startIndex - 1; i >= 0; i--) {
			...
			if (char === ',') {
				...
				// If we hit a non-whitespace, non-comma character, stop looking
				break;
			}
		}

		// Look for trailing comma (might be after the closing brace)
		let actualEndIndex = endIndex;
		for (let i = endIndex; i < featuresContent.length; i++) {
			const char = featuresContent[i];
			if (char === ',') {
				actualEndIndex = i + 1;
				break;
		...
		let beforeItem = featuresContent.substring(0, actualStartIndex);
		let afterItem = featuresContent.substring(actualEndIndex);

		// Clean up trailing whitespace and commas in beforeItem
		beforeItem = beforeItem.replace(/,\s*$/, '').trimEnd();

		// Clean up leading whitespace (but keep one newline for formatting)
		afterItem = afterItem.replace(/^\s+/, '\n');

		featuresContent = beforeItem + afterItem;
	}
} else {
	console.log(`⚠ Warning: Could not find feature item with id '${variationKebab}' in features.js`);
}
