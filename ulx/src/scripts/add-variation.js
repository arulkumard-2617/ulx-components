...
if (fs.existsSync(importSnippetPath)) {
  ...
  const importMatch = importSnippetContent.match(/import\s+(?:\{([^}]+)\}|(\w+))\s+from\s+['"]uls-components['"]/);
  if (importMatch) {
    actualComponentImportName = importMatch[1] ? importMatch[1].trim() : importMatch[2];
    actualComponentPath = toKebabCase(actualComponentImportName);
  ...
  if (fs.existsSync(basicDemoPath)) {
    ...
    const importMatch = basicDemoContent.match(/import\s+(?:\{([^}]+)\}|(\w+))\s+from\s+['"]uls-components['"]/);
    if (importMatch) {
      actualComponentImportName = importMatch[1] ? importMatch[1].trim() : importMatch[2];
      actualComponentPath = toKebabCase(actualComponentImportName);
    ...
    if (demoImportMatch) {
      ...
      if (existingDemoFile) {
        ...
        const importMatch = existingDemoContent.match(/import\s+(?:\{([^}]+)\}|(\w+))\s+from\s+['"]uls-components['"]/);
        if (importMatch) {
          actualComponentImportName = importMatch[1] ? importMatch[1].trim() : importMatch[2];
          actualComponentPath = toKebabCase(actualComponentImportName);
          actualComponentTagName = actualComponentImportName;
