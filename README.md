...
Built styles (`dev-releases/css/ulx-editor.min.css` and `dev-releases/css/ulx-community.min.css`) are **included in the published npm package** (`package.json` `files` includes `dev-releases`). They are produced by **`npm run build:css`** at the repo root (or `npm run ulxEditor` from `ulx/`; see `ulx/package.json`), then Rollup via **`npm run build`** (`build:css` then `build:js`). The minified files are gitignored. Published packages omit `ulx/` and ship pre-built `dev-releases/` instead.
