...
Built styles (`dev-releases/css/ulx-editor.min.css`) are **included in the published npm package** (`package.json` `files` includes `dev-releases`). They are produced by `node ulx/src/scripts/less/ulx-editor-build.js`, which also runs as part of `npm run build` and **`npm pack` / `npm publish` via `prepack`**.
