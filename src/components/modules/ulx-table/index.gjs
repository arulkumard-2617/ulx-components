...
export default class UlxTable extends Component {
  ...
  handleSort(field) {
    ...
    if (this.sortMode === 'multiple') {
      ...
      if (this.sortField === field) {
        ...
        else if (removableSort) { this._sortField = null; this._sortOrder = 1; if (lazy) this.args.onSort?.({ field: null, order: null }); return; }
        else order = 1;
      } else {
        order = 1;
      }
  ...
  handleManageColumnsApply({ columns }) {
    const dataFields = new Set(
      columns.filter((c) => c.field && !c.selectionMode && !c.expander && !c.rowReorder && !c.rowEditor).map((c) => c.field)
    );
