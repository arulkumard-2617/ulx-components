...
export default class ColorPaletteComponent extends Component {
  ...
  <template>
    <div class="fxb fcol gp15 w-100p">
      {{#each @paletteGroups as |group|}}
        <div class="fxcol gp4">
          <h5 class="mgt0 mgb4 font-bold fg-primary">{{group.title}}</h5>
          <div class="uls-grid col-4">
            {{#each group.colors as |color|}}
              <article class="uls-foundation-card pd6 rds2 bdr flex-column md-w-1-3 text-center">
                <div
                  class="rds2 mgb4 bdr mg-auto"
