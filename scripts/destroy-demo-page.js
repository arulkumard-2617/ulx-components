...
// Filter out 'destroy-demo-page' if present (when called from wrapper)
const filteredArgs = args.filter(arg => arg !== 'destroy-demo-page');
...
if (!componentName) {
  ...
  console.error('Usage: npm run destroy demo-page ComponentName --category collections');
  process.exit(1);
}

// Parse flags
const categoryIndex = filteredArgs.indexOf('--category');
const category = categoryIndex !== -1 && filteredArgs[categoryIndex + 1] ? filteredArgs[categoryIndex + 1] : 'collections';
...
// 6. Update router.js - remove route
let routerContent = fs.readFileSync(routerPath, 'utf8');
const routeLinePattern = new RegExp(`\\s*this\\.route\\('${actualKebabName}', \\{ path: '/${actualKebabName}' \\}\\);?\\s*\\n?`, 'g');
if (routeLinePattern.test(routerContent)) {
  routerContent = routerContent.replace(routeLinePattern, '');
...
if (routeMatch) {
  ...
  if (startIndex !== -1 && endIndex !== -1) {
    ...
    // Clean up empty submodule categories (if all items removed from a submodule)
    // Match: { category: '...', items: [] }
    const emptySubmodulePattern = new RegExp(
      `\\s*\\{[\\s\\S]*?category:[\\s\\S]*?items:\\s*\\[\\s*\\]\\s*\\},?\\s*\\n?`,
      'g'
    );
    docsContent = docsContent.replace(emptySubmodulePattern, '');
    
    // Clean up empty children arrays (if all items removed from a category)
    docsContent = docsContent.replace(/children:\s*\[\s*\]/g, 'children: []');
    
    // Clean up trailing commas before closing brackets ONLY
    // Only remove commas that are directly before ] or } that closes an array/object
    // Do NOT remove commas before opening braces { as those are needed between array items
    docsContent = docsContent.replace(/,\s*(\s*\])/g, '$1');
    // Only remove comma before } if it's closing an array item (followed by ] or another })
    docsContent = docsContent.replace(/,\s*(\s*\}(?=\s*\]))/g, '$1');
