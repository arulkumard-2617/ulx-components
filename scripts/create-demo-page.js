 ...
 * Usage: npm run create demo-page PageName[TabMenu] --category collections --submodule menu
 * Example: npm run create demo-page TabMenu[TabMenu] --category collections --submodule menu
 */
...
if (!pageName) {
  ...
  console.error('Usage: npm run create demo-page PageName[TabMenu] --category collections --submodule menu');
  process.exit(1);
}
...
const category = categoryIndex !== -1 && args[categoryIndex + 1] ? args[categoryIndex + 1] : 'collections';
const submodule = submoduleIndex !== -1 && args[submoduleIndex + 1] ? args[submoduleIndex + 1] : null;
...
[routesPath, templatesPath, controllersPath, documentationPath, demoComponentsPath].forEach(dir => {
...
export default {
  ...
  importMsg: "import ${pascalName} from 'uls-components/components/${category}/${kebabName}'",
...
if (routerContent.includes(`this.route('${kebabName}'`)) {
  ...
  const componentsRoutePattern = /(this\.route\('components', \{ path: '\/components' \}, function \(\) \{[\s\S]*?)(\}\);)/;
  if (componentsRoutePattern.test(routerContent)) {
    ...
    const categoryRouteStart = new RegExp(`this\\.route\\('${category}', \\{ path: '/${category}' \\}, function \\(\\) \\{`);
    ...
    if (categoryMatch) {
      ...
      // Find the position before the closing brace (where we'll insert the new route)
      // Look for the last route statement before the closing brace
      const categoryBlock = routerContent.substring(startIndex, endIndex);
      ...
      // Add the new route with proper indentation
...
// Find the category in the navigation
const categoryPattern = new RegExp(`(menuTitle: '${categoryPascal}',[\\s\\S]*?children: \\[[\\s\\S]*?)(\\]\\s*\\})`, 'm');
...
if (categoryPattern.test(docsContent)) {
  ...
  if (submodule) {
    // Check if submodule category exists
    const submodulePattern = new RegExp(`(category: '${toPascalCase(submodule)}',[\\s\\S]*?items: \\[[\\s\\S]*?)(\\]\\s*\\})`, 'm');
    if (submodulePattern.test(docsContent)) {
      // Add to existing submodule
      docsContent = docsContent.replace(submodulePattern, (match, p1, p2) => {
        ...
        if (lastBraceIndex !== -1) {
          ...
          // Look for comma in the first few characters (handling whitespace/newlines)
          const afterBraceTrimmed = afterLastBrace.trim();
          // Check if it starts with comma, or if the first non-whitespace char after } is ]
          const hasComma = afterBraceTrimmed.startsWith(',') || 
                          (afterLastBrace.match(/^\s*[,]/) !== null);
          ...
          if (hasComma) {
            // Already has comma, just add new item after the existing comma
            // Format: },\n          {newItem},\n        ]
            return `${beforeLastBrace}}${afterLastBrace}\n${newItem},\n        ${p2}`;
          } else if (nextCharIsBracket) {
            // Next char is ], so we're at the end - add comma and new item before ]
            return `${beforeLastBrace}},\n${newItem}\n        ${p2}`;
          } else {
            // No comma and not at end (might have whitespace/newlines before next item)
      ...
      // Add new submodule
      docsContent = docsContent.replace(categoryPattern, `$1${menuItem},\n      $2`);
    }
  } else {
...
console.log(`2. Add more demo components and features in app/documentation/components/${category}/${kebabName}/`);
