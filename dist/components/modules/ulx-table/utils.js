 ...
 */
function sortItems(items, sortField, sortOrder = 1) {
 ...
 * @returns {Array} new sorted array
 */
function multiSortItems(items, multiSortMeta) {
  if (!items) return [];
 ...
 * @returns {Array}
 */
function filterItems(items, filters, globalFilterFields) {
  if (!items) return [];
    ...
    for (const [key, filterMeta] of Object.entries(filters)) {
      if (key === 'global') {
        ...
        const matched = fields.some(f => {
          ...
          return matchesConstraint(cellValue, {
            value: globalValue,
            matchMode
          });
        });
 ...
 */
function paginateItems(items, first, rows) {
  if (!items) return [];
...
function saveTableState(key, storage = 'session', state) {
  if (!key) return;
  try {
...
function loadTableState(key, storage = 'session') {
  if (!key) return null;
  try {
    const store = storage === 'local' ? localStorage : sessionStorage;
...
function exportCSV(columns, data, filename = 'export.csv') {
  ...
  const rows = data.map(row => exportCols.map(c => JSON.stringify(String(getFieldValue(row, c.field) ?? ''))).join(','));
...
function parseSortBy(sortByStr) {
  if (!sortByStr || typeof sortByStr !== 'string') return {
    field: null,
    order: 1
  };
...
export { compareValues, exportCSV, filterItems, getFieldValue, isSpecialColumn, loadColumnWidths, loadTableState, multiSortItems, paginateItems, parseSortBy, reorderArray, saveColumnWidths, saveTableState, sortItems };
