...
function validateFormat(value, formatRule) {
  ...
  if (!formatRule.with.test(str)) {
    return typeof formatRule.msg === 'string' && formatRule.msg.length > 0 ? formatRule.msg : 'Invalid value';
  }
}
...
function validate(context, validations) {
  ...
  for (const key of Object.keys(validations ?? {})) {
    ...
    if (rules.required != null && rules.required !== false) {
      if (trimmed === '') {
        errors[key] = typeof rules.required === 'string' && rules.required.length > 0 ? rules.required : 'Required';
        continue;
      }
    }
    ...
    if (typeof maxLen === 'number' && str.length > maxLen) {
      errors[key] = typeof rules.maxLength?.msg === 'string' && rules.maxLength.msg.length > 0 ? rules.maxLength.msg : `Maximum ${maxLen} characters allowed`;
      continue;
    }
    const minLen = getConstraintValue(rules, 'minLength');
    if (typeof minLen === 'number' && str.length < minLen) {
      errors[key] = typeof rules.minLength?.msg === 'string' && rules.minLength.msg.length > 0 ? rules.minLength.msg : `Minimum ${minLen} characters required`;
    }
  }
  return {
