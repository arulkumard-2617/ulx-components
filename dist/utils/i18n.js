...
const enUs = {
  ...
  'msg.popup.overlay.options.doc': 'Use @context to choose where the popup is rendered. You can keep it inline with the trigger or portal it to the document body.',
  'msg.popup.overlay.scroll.help': 'Open the popup inside this scrollable container, then scroll to see @scrollContext keep it aligned while it stays open.',
  'msg.popup.positions.help': 'Click a button to open the popup at that position. This demo keeps the popup in the body context so each placement is easy to compare.',
  ...
  'msg.dropdown.overlay.options.doc': 'Use <code>@context</code> to choose where the panel is rendered. Combine <code>@boundary</code> and <code>@scrollContext</code> to keep the overlay inside a scrollable container and reposition it while that container scrolls.',
  'msg.dropdown.overlay.scroll.help': 'Open the dropdown below, then scroll this container to see <code>@scrollContext</code> keep the panel aligned while it stays open.',
  ...
  'msg.doc.section.work.in.progress.body': 'Documentation for this tab is not available yet and will be added in a future update.',
  ...
  'msg.multiselect.tags.help': 'Use keywords or phrases that describe your event to help everyone find it easily. Separate multiple tags with commas.',
  // ── Tristate checkbox ────────────────────────────────────
  ...
  'msg.router.tabmenu.description': 'This demo shows TabMenu with LinkTo routing. Click tabs to navigate between routes. The active tab is automatically synced with the current route.',
  // ── Segment labels ───────────────────────────────────────
  ...
  'msg.card.advanced.desc': 'Card with custom header and footer based on PrimeReact Card advanced example.',
  'msg.card.accessibility.desc': 'Use role="region" with aria-label when the card should be announced as a landmark.',
  'msg.card.basic.body': 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Inventore sed consequuntur error repudiandae numquam deserunt quisquam repellat libero asperiores earum.',
...
function setLocale(locale) {
  if (!_locales[locale]) {
    throw new Error(`[ulx-i18n] Locale "${locale}" is not registered. ` + `Call addTranslations("${locale}", translations) first.`);
  }
...
export { addTranslations, getLocale, hasTranslation, setLocale, t, tSafe };
