 ...
 * @returns {boolean}
 */
function isTopModalInstance(componentInstance) {
 ...
 */
function resolveOverlayInitialFocus(maskElement, overlaySelector, initialFocusSelector) {
 ...
 * @returns {void}
 */
function scheduleOverlayInitialFocus(maskElement, overlaySelector, initialFocusSelector, delayMs) {
  setTimeout(() => {
 ...
 * @returns {boolean}
 */
function isPointerOutsideRootAndPanel(rootElement, event, panelElement) {
 ...
 * @returns {boolean}
 */
function isPointerOutsideAnchoredOverlay(rootElement, anchorElement, event) {
  const target = event.target;
  const outsideRoot = rootElement && !rootElement.contains(target);
  const outsideAnchor = anchorElement && !(anchorElement === target || anchorElement.contains(target));
 ...
 * @returns {boolean}
 */
function isPointerOutsideElement(containerElement, event) {
 ...
 * @returns {void}
 */
function handleAsyncAction(action, options = {}) {
  if (!action) return;
  const {
    setSubmitting,
    onSuccess,
 ...
 * @returns {void}
 */
function handleTabKey(event, overlayElement) {
 ...
 */
function getDocumentScrollOffsets() {
 ...
 * @returns {void}
 */
function applyBodyAbsoluteFromViewport(containerElement, topViewport, leftViewport) {
 ...
 * @returns {number}
 */
function getOverlayZIndexAboveMask(modalStack, instance) {
 ...
 * @returns {boolean}
 */
function shouldShowOverlay(visible, shouldRender, setShouldRender) {
 ...
 * @returns {Object}
 */
function buildOverlayLifecycleOptions(component, overrides = {}) {
  const {
...
export { FOCUSABLE_SELECTOR, applyBodyAbsoluteFromViewport, buildOverlayLifecycleOptions, getDestinationElement, getDocumentScrollOffsets, getOverlayZIndexAboveMask, handleAsyncAction, handleTabKey, hasClosableToastInDom, isEscapeKey, isPointerOutsideAnchoredOverlay, isPointerOutsideElement, isPointerOutsideRootAndPanel, isTopModalInstance, resolveOverlayInitialFocus, scheduleOverlayInitialFocus, shouldShowOverlay };
//# sourceMappingURL=overlay-helpers.js.map
