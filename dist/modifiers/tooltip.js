 ...
 */
var tooltip = modifier(function tooltip(element, [content, optionsArg], {
  position = 'right',
  event = 'both',
  showDelay = 0,
  hideDelay = 0,
  ...
  const appendTarget = opts.appendTo ?? appendTo ?? (typeof document !== 'undefined' ? document.body : null);
  ...
  const addedTabindex = evt === 'both' || evt === 'focus' ? ensureFocusableForTooltip(element) : false;
  function getAppendTarget() {
    if (appendTarget === document.body) return document.body;
    if (typeof appendTarget === 'string') return document.querySelector(appendTarget) ?? document.body;
    return appendTarget;
  }
  ...
  function onEscape(e) {
    if (e.key === 'Escape' && escapeClose && tooltipEl && tooltipEl.getAttribute('aria-hidden') !== 'true') {
      e.preventDefault();
      doHide();
    }
  }
